...
function keccak(bytes) {
  return new Uint8Array(_jssha3.keccak256.digest(bytes));
}
...
var _hpackjs = require('hpack.js'); var _hpackjs2 = _interopRequireDefault(_hpackjs);
...
function fieldHpackStaticTableIndex(name) {
  const elem = _hpackjs2.default["static-table"].table.find(
    (row) => row.name === name.toLowerCase()
  );
  if (!elem)
    return null;
  return _hpackjs2.default["static-table"].table.indexOf(elem);
}

// src/metadata/encode.ts
function encodeMetadata(metadata) {
  const comp = _hpackjs2.default.compressor.create({ table: { size: 256 } });
  let headers = [];
  let name, value;
...
async function prepareFile(file, chunkSize = DEFAULT_CHUNK_SIZE) {
  ...
  let mime = _mimetypes.lookup.call(void 0, name);
  if (!mime) {
  } else {
    metadata["Content-Type"] = mime;
  }
  const compressed = _pako.gzip.call(void 0, content);
...
function decodeMetadata(raw) {
  const decomp = _hpackjs2.default.decompressor.create({ table: { size: 256 } });
  const metadata = {};
  decomp.write(raw);
  decomp.execute();
  let buff;
...
function createProxyResolver(resolver) {
  return async (uri) => {
    try {
      ...
      let paths = _optionalChain([path, 'optionalAccess', _2 => _2.split, 'call', _3 => _3("/")]) || [];
...
exports.ProxyResolutionStatusErrors = ProxyResolutionStatusErrors; exports.ProxyResolutionStatusRedirect = ProxyResolutionStatusRedirect; exports.ProxyResolutionStatusSuccess = ProxyResolutionStatusSuccess; exports.blockchainNames = blockchainNames; exports.default = src_default;
